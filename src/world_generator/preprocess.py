...
class OSMPreprocessor(osmium.SimpleHandler):
    ...
    def get_writer(self, filename: str):
        return osmium.SimpleWriter(os.path.join(self.output_folder, filename))
...
def preprocessOSM():
    ...
    osm_files_exist = all(
        os.path.exists(os.path.join(output_folder, f"{name}.osm"))
        for name in ALL_OSM_FILES
    )
    ...
    all_output_files_exist = all(
        os.path.exists(os.path.join(output_folder, f"{name}.shp"))
        for name in OSM_POSTFIX
    )
...
def QGISfix(output_name: str):
    input_file = os.path.join(
        CONFIG["osm_folder_path"], "all/", OSM_POSTFIX[output_name][0] + ".osm"
    )
    output_file = os.path.join(CONFIG["osm_folder_path"], "all/", output_name + ".shp")

    o = fix_geometry(
        "",
        "native:fixgeometries",
        {"INPUT": input_file + OSM_POSTFIX[output_name][1], "OUTPUT": output_file},
    )
