...
logger = logging.getLogger("wpscript")


def runWorldPainter(
    tile: str, degree_per_tile: int, blocks_per_tile: int, height_ratio: int
):
    # tile: N01E000
    ...
    o = subprocess.run(
        [
            "minutor",
            "--world",
            os.path.join(CONFIG["scripts_folder_path"], "wpscript", "exports", tile),
            "--depth",
            "319",
            "--savepng",
...
def wpGenerate():
    ...
    for xMin in range(-180, 180, degree_per_tile):
        for yMin in range(-90, 90, degree_per_tile):
            ...
            pool.schedule(
                runWorldPainter, (tile, degree_per_tile, blocks_per_tile, height_ratio)
            )
    pool.close()
    pool.join()
